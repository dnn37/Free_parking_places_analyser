...
class ShowNode:
    ...
    def __init__(self, config) -> None:
        ...
        self.max_cars_amount_parking1 = config["general"]["road1_max"]
    ...
    def process(self, frame_element: FrameElement, fps_counter=None) -> FrameElement:
        ...
        if self.show_roi:
            for road_id, points in frame_element.roads_info.items():
                ...
                # Отображение номера дороги в незалитой окружности
        ...
        if self.show_info_statistics:
            ...
            y = 55 
            # Текст для максимального количества машин
            text_cars = f"Max parking#1 places: {self.max_cars_amount_parking1}"
            ...
            y += 2*(cv2.getTextSize(text_cars, self.fontFace, self.fontScale*1.5, self.thickness)[0][1] + 25)
            
            # Текст для заголовка
            text_info = "Free parking places"
            # Выводим заголовок
            cv2.putText(
                img=black_image,
                text=text_info,
            ...
            if frame_element.timestamp >= self.buffer_analytics_sec:
                ...
                for key, value in data_info.items():
                    ...
                    value = int(value)
                    text_road = f"  parking#{key}: {value} cars"
                    cv2.putText(
                        img=black_image,
                        text=text_road,
                        org=(20, y),
