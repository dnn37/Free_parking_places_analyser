...
class SendInfoDBNode:
    ...
    def __init__(self, config: dict) -> None:
        ...
        self.max_parking1_places = config["general"]["road1_max"]
        ...
        CREATE TABLE {self.table_name} (
            ...
            timestamp INTEGER,
            timestamp_date TIMESTAMP WITH TIME ZONE,
            parking1_places INTEGER,    
            max_parking1_places INTEGER   
        );
        """
    ...
    def process(self, frame_element: FrameElement) -> FrameElement:
        ...
        max_parking1_places = self.max_parking1_places

       
        # Проверка, нужно ли отправлять информацию в базу данных
        current_time = time.time()
        if current_time - self.last_db_update >= self.how_often_add_info:
            self._insert_in_db(info_dictionary, timestamp, timestamp_date,max_parking1_places)
    ...
    def _insert_in_db(self, info_dictionary: dict, timestamp: float, timestamp_date: float,max_parking1_places:int ) -> None:
        # Формирование и выполнение SQL-запроса для вставки данных в бд
        insert_query = (
            f"INSERT INTO {self.table_name} "
            "(timestamp, timestamp_date, parking1_places,max_parking1_places) "
        ...
        try:
            self.cursor.execute(
                ...
                (
                    ...
                    (
                        info_dictionary[1]
                        if timestamp >= self.buffer_analytics_sec #для набора статистики в начале работы
                        else None
                    ),
                    max_parking1_places,
                 ),
            ...
            print("Запись в БД не произошла")
            logger.error(
